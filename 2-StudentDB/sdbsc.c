...
int add_student(int fd, int id, char *fname, char *lname, int gpa)
{
    ...
    lseek(fd, offset, SEEK_SET);
    int bytes_read = read(fd, &existing_student, sizeof(student_t));

    if (bytes_read == sizeof(student_t)) {
        for (size_t i = 0; i < sizeof(student_t); i++) {
            if (((char*)&existing_student)[i] != 0) {
                is_empty = false;
                break;
            }
        }
    ...
    lseek(fd, offset, SEEK_SET);
    int bytes_written = write(fd, &student, sizeof(student_t));
...
student_t student;
    ...
    while (read(fd, &student, STUDENT_RECORD_SIZE) == STUDENT_RECORD_SIZE) {
        if (student.id == id) {
            // Mark as deleted (you can set all fields to EMPTY_STUDENT_RECORD if necessary)
            student.id = DELETED_STUDENT_ID;  // Or set the whole record to an empty one
            lseek(fd, offset, SEEK_SET);
...
int print_db(int fd)
{
    ...
    while (read(fd, &student, sizeof(student_t)) > 0) {
        // Ensure valid student record before printing
        if (student.id != 0 && memcmp(&student, &EMPTY_STUDENT_RECORD, sizeof(student_t)) != 0) {
            if (!printed_header) {
