...
int clear_command_list(command_list_t *cmd_list) {
    ...
    for (int i = 0; i < CMD_PIPELINES_MAX; i++) {
...
int build_command_list(char *line, command_list_t *cmd_list) {
    clear_command_list(cmd_list);
    
    char *token = strtok(line, "|");
    int cmd_count = 0;
    
    while (token != NULL && cmd_count < CMD_PIPELINES_MAX) {
        while (*token == ' ') {
            token++;
        }
...
int exec_piped_commands(command_list_t *cmd_list) {
    ...
    int pipes[CMD_PIPELINES_MAX][2];
    pid_t child_pids[CMD_PIPELINES_MAX];
    ...
    for (int i = 0; i < num; i++) {
        ...
        if (pid == 0) { 
            ...
            else if (cmd->input_redirect != NULL) {
            ...
            } else if (cmd->output_redirect != NULL) {
...
int exec_local_cmd_loop() {
    ...
    while (1) {
        ...
        rc = build_command_list(line_copy, &cmd_list);
        ...
        if (rc == WARN_NO_CMDS) {
            ...
            fprintf(stderr, CMD_ERR_PIPE_LIMIT, CMD_PIPELINES_MAX);
            continue;
        } 
